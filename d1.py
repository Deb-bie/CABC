...
img_size = 224  # Standard size for DeiT as well
batch_size = 48
epochs = 30
...
def train_model():
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_temp, y_temp)):
        ...
        callbacks = [
            ...
            ReduceLROnPlateau(
                ...
                factor=0.5,
                patience=5,
                min_lr=1e-7,  # Lower min_lr for transformers
                verbose=1
            ),
    ...
    # Save best model
    best_model.save('best_histopathology_deit_model.h5')
    print("Best model saved as 'best_histopathology_deit_model.h5'")
